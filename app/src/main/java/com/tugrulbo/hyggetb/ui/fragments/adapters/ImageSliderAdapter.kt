...
class ImageSliderAdapter(private val context: Context?, private var sliders: List<Category>?) : PagerAdapter() {
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        ...
        var imageUrl = sliders?.get(position)?.imageUrl
        Picasso.get().load(imageUrl).resize(1300,1000).into(view.iVSliderImage)

        /*view.setOnClickListener(View.OnClickListener {
            var id = sliders?.get(position)?.id
